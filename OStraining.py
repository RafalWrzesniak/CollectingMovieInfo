...
def dataProcessing():
    ...
    # wholeList = [{'Tytuł': ['1492: Wyprawa do raju'], 'Długość': [154], 'Premiera': ['1992-10-08'], 'Gatunek': ['Biograficzny', 'Przygodowy', 'Dramat historyczny'], 'Produkcja': ['Francja', 'Hiszpania'], 'Reżyseria': ['Ridley Scott'], 'Obsada': ['Gerard Depardieu', 'Armand Assante', 'Sigourney Weaver'], 'Filmweb link': ['http://www.filmweb.pl/film/1492%3A+Wyprawa+do+raju-1992-985'], 'Opis': ['Rok 1492, Krzysztof Kolumb wyrusza w podróż pod patronatem hiszpańskiej królowej Izabeli. Cel wyprawy to odnalezienie drogi morskiej do Indii.']}, {'Tytuł': ['2001: Odyseja kosmiczna'], 'Długość': [141], 'Premiera': ['1968-04-02'], 'Gatunek': ['Sci-Fi'], 'Produkcja': ['USA', 'Wielka Brytania'], 'Reżyseria': ['Stanley Kubrick'], 'Obsada': ['Keir Dullea', 'Gary Lockwood', 'William Sylvester'], 'Filmweb link': ['http://www.filmweb.pl/2001.Odyseja.Kosmiczna'], 'Opis': ['Film dzieli się na trzy epizody, których akcja ma miejsce w różnych okresach historycznych i zakątkach wszechświata. ']}, {'Tytuł': ['Ben Hur'], 'Długość': [212], 'Premiera': ['1959-11-18'], 'Gatunek': ['Dramat', 'Kostiumowy'], 'Produkcja': ['USA'], 'Reżyseria': ['William Wyler'], 'Obsada': ['Charlton Heston', 'Jack Hawkins', 'Haya Harareet'], 'Filmweb link': ['http://www.filmweb.pl/Ben.Hur'], 'Opis': ['Jerozolima, początek naszej ery. Wygnany z kraju przez swojego przyjaciela izraelski książę spotyka na pustyni Chrystusa. Jego nauka odmieni życie bohatera.']}, {'Tytuł': ['Big Lebowski'], 'Długość': [117], 'Premiera': ['1998-02-15'], 'Gatunek': ['Komedia kryminalna'], 'Produkcja': ['USA', 'Wielka Brytania'], 'Reżyseria': ['Joel Coen'], 'Obsada': ['Jeff Bridges', 'John Goodman', 'Julianne Moore'], 'Filmweb link': ['http://www.filmweb.pl/film/Big+Lebowski-1998-13'], 'Opis': ['Jeffrey Lebowski postanawia odnaleźć swojego bogatego imiennika w celu uzyskania rekompensaty za zniszczony przez włamywaczy dywan.']}, {'Tytuł': ['Donnie Darko'], 'Długość': [113], 'Premiera': ['2001-01-19'], 'Gatunek': ['Dramat'], 'Produkcja': ['USA'], 'Reżyseria': ['Richard Kelly'], 'Obsada': ['Jake Gyllenhaal', 'Jena Malone', 'Holmes Osborne'], 'Filmweb link': ['http://www.filmweb.pl/Donnie.Darko'], 'Opis': ['Donnie Darko, uważany za nastolatka z zaburzeniami osobowości, spotyka Franka - tajemniczą postać w kostiumie królika, która zaczyna manipulować jego życiem. ']}]
        ...
        for movieDict1 in wholeList:
            for key in movieDict1:
                ...
                if len(movieDict1[key]) > 1:
                    ...
                    if key == 'Długość':
                        saveFile.write(str(movieDict1[key][0] // 60) + ' godz. ' + str(movieDict1[key][0] % 60) + ' min.' + ';')
                    else:
                        saveFile.write(str(movieDict1[key][0]) + ';')
            saveFile.write('\n')
        saveFile.close()
        print(bold, "Execution time: ", end, + str(round(time.time() - Astart, 2)) + " [s].", bold, "Ilość filmów: ", end, str(len(wholeList )))
...
Astart = time.time()
SzukaniePoFolderach()
#movieList = ["Millennium: Mezczyzni, ktorzy nienawidza kobiet", "Most szpiegów", "Wilk", "Kiler"]
